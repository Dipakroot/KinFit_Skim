...
void Selector::filter_electrons(EventTree *event){
  ...
  for(int eleInd = 0; eleInd < int(tree->nEle_) and int(tree->nEle_) <= 20 ; ++eleInd){
    ...
    bool passMVAIsoLooseID = tree->eleIDmvaIsoLoose_[eleInd]; 
   ...
   // bool passMVAWP80ID = tree->eleIDmvaWP80_[eleInd];
    ...
    cout<<"passMVAIsoLooseID ="<<passMVAIsoLooseID<<"  passMVALooseID="<<passMVALooseID<<endl;
...
void Selector::filter_taus(EventTree *event){
  ...
  for(int tauInd = 0; tauInd < int(tree->nTau_) && int(tree->nTau_) <= 15; ++tauInd){
    ...
    bool hadronicTau = (decayMode ==0 || decayMode==1 || decayMode==2 || decayMode== 10 || decayMode== 11) ; 
    ...
    bool TauIDTest =( tauIdDeepTau2017v2p1VSjet >=16 &&
		      tauIdDeepTau2017v2p1VSe >= 2 && 
		      tauIdDeepTau2017v2p1VSmu >= 1
	        	   );
    ...
    bool passTest = (hadronicTau && generalTau && TauIDTest );
    ...
    */
    if(generalTau)
	Taus_test1.push_back(tauInd);
    if(generalTau && hadronicTau)
	Taus_test2.push_back(tauInd);

    if(passTest){
      Taus.push_back(tauInd); 
     // if(tree->event_==5)
//	cout<<"Event_number=5: Tau information : Tau pt="<< pt <<"Tau Eta="<<eta<<endl;
    }
    if(passLoose)
...
void Selector::filter_jets(EventTree *event, int tauId_1, int tauId_2){
    tree = event;
    	cout<<"inside from jet filter"<<endl;
  ...
  //  if(tree->event_==5)
    //  cout<<"Event_number=5: jet information : jet pt="<< pt <<"jet Eta="<<eta<<"jet Phi"<<phi<<endl;
  ...
  //  if(pt<=jet_Pt_cut) continue;
   ...
   // int jetID_cutBit = 1;
    ...
    //Applied for UL
    bool jetID_pass = (tree->jetID_[jetInd]>=6) ;    
    //Applied for UL
	...
	*/
    ...
    if(pt >= jet_Pt_cut && TMath::Abs(eta) <= jet_Eta_cut)
                Jets_test1.push_back(jetInd);
    if(pt >= jet_Pt_cut && TMath::Abs(eta) <= jet_Eta_cut && jetID_pass)
                Jets_test2.push_back(jetInd);
   if(pt >= jet_Pt_cut && TMath::Abs(eta) <= jet_Eta_cut && jetID_pass && passDR_lep_jet)
                Jets_test3.push_back(jetInd); 


   if( jetSel ){
     // if(tree->event_==5)
	//	cout<<"After cleaning:Event_number=5: jet information : jet pt="<< pt <<"jet Eta="<<eta<<"jet Phi"<<phi<<endl;
