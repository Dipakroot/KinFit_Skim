...
Int_t SkimAna::CreateHistoArrays()
{
  ...
  for(int isyst=0;isyst<fNSyst;isyst++){
    ...
    d2->cd();
    fFileDir[isyst*fNDDReg + 0] = d2->mkdir("Tau_Cutflow");
    ...
    */
    ...
    int hidx = 0;
    hCutFlow[fNCFHists*isyst + hidx++] = new TH1D("_cutflow_tau", "_cutflow_tau", 8, 0, 8);
    //cout<<"222"<<endl;
    ...
    */
    ...
    histObs[fNObHists*isyst + hidx++] = new TH1D("pt_tau_1","pt_tau_1",100, 0., 1000.);
    
    histObs[fNObHists*isyst + hidx++] = new TH1D("eta_tau_1","eta_tau_1", 30, -3., 3.);
    histObs[fNObHists*isyst + hidx++] = new TH1D("phi_tau_1","phi_tau_1", 35, -3.5, 3.5);
    histObs[fNObHists*isyst + hidx++] = new TH1D("charge_tau_1","charge_tau_1", 10, -2., 2.);

    histObs[fNObHists*isyst + hidx++] = new TH1D("pt_tau_2","pt_tau_2",100, 0., 1000.);
    histObs[fNObHists*isyst + hidx++] = new TH1D("eta_tau_2","eta_tau_2", 30, -3., 3.);
    histObs[fNObHists*isyst + hidx++] = new TH1D("phi_tau_2","phi_tau_2", 35, -3.5, 3.5);
    histObs[fNObHists*isyst + hidx++] = new TH1D("charge_tau_2","charge_tau_2", 10, -2., 2.);

    histObs[fNObHists*isyst + hidx++] = new TH1D("mass_ditau","mass_ditau",100, 0., 1000.);
    histObs[fNObHists*isyst + hidx++] = new TH1D("charge_ditau","charge_ditau", 10, -2., 2.);
    histObs[fNObHists*isyst + hidx++] = new TH1D("Sum_Pt_ditau","Sum_Pt_ditau",100, 0., 1000.);

    histObs[fNObHists*isyst + hidx++] = new TH1D("nTau","nTau",10, 0., 10.);
    histObs[fNObHists*isyst + hidx++] = new TH1D("nJet","nJet",20, 0., 20.);
    histObs[fNObHists*isyst + hidx++] = new TH1D("nBJet","nBJet",10, 0., 10.);
  
   histObs[fNObHists*isyst + hidx++] = new TH1D("nEleLoose","nEleLoose",10, 0., 10.);
   histObs[fNObHists*isyst + hidx++] = new TH1D("nMuLoose","nMuLoose",10, 0., 10.);
   histObs[fNObHists*isyst + hidx++] = new TH1D("nJet_test1","nJet_test1",20, 0., 20.); 
   histObs[fNObHists*isyst + hidx++] = new TH1D("nJet_test2","nJet_test2",20, 0., 20.);
   histObs[fNObHists*isyst + hidx++] = new TH1D("nJet_test3","nJet_test3",20, 0., 20.);
  ...
  if(fSyst=="base"){
    ...
    hControl[iscl*fNBSelColHists + hidx++] = new TH1D("phi_tau_1","phi_tau_1", 35, -3.5, 3.5);
    hControl[iscl*fNBSelColHists + hidx++] = new TH1D("charge_tau_1","charge_tau_1", 10, -2., 2.);

    hControl[iscl*fNBSelColHists + hidx++] = new TH1D("pt_tau_2","pt_tau_2",100, 0., 1000.);
    hControl[iscl*fNBSelColHists + hidx++] = new TH1D("eta_tau_2","eta_tau_2", 30, -3., 3.);
    hControl[iscl*fNBSelColHists + hidx++] = new TH1D("phi_tau_2","phi_tau_2", 35, -3.5, 3.5);
    hControl[iscl*fNBSelColHists + hidx++] = new TH1D("charge_tau_2","charge_tau_2", 10, -2., 2.);

    hControl[iscl*fNBSelColHists + hidx++] = new TH1D("mass_ditau","mass_ditau",100, 0., 1000.);
    hControl[iscl*fNBSelColHists + hidx++] = new TH1D("charge_ditau","charge_ditau", 10, -2., 2.);
    hControl[iscl*fNBSelColHists + hidx++] = new TH1D("Sum_Pt_ditau","Sum_Pt_ditau",100, 0., 1000.);

    hControl[iscl*fNBSelColHists + hidx++] = new TH1D("nTau","nTau",10, 0., 10.);
    hControl[iscl*fNBSelColHists + hidx++] = new TH1D("nJet","nJet",20, 0., 20.);
    hControl[iscl*fNBSelColHists + hidx++] = new TH1D("nBJet","nBJet",10, 0., 10.);
...
void SkimAna::SetTrio()
{
  ...
  if(fYear==2016){
    ...
    //selector->jet_Pt_cut = 25.;
    selector->jet_Pt_cut = 30.;
    selector->jet_Eta_cut = 5.0;
    selector->tau_Pt_cut = 40.0;
    selector->tau_Eta_cut = 2.1;
  }
...
Bool_t SkimAna::Process(Long64_t entry)
{
  ...
  /*
	if(
     ...
     //and
     (!(evtPick->passFilter) or !(selector->isPVGood) 
     ) return true;
 */ 
  //cout<<"isPVGood="<<selector->isPVGood<<endl;
  FillEventCutFlow();//edited above line to check gen matching
  FillEventWt();
 ...
 // if(IsDebug) Info("Process","Completed Event filling");
  ...
  _nTau =selector->Taus.size();
  	  //checking veto ele/mu numbers
   // if(selector->MuonsLoose.size()>=1)
	N_veto_ele=0; N_veto_mu=0;
	N_veto_mu=selector->MuonsLoose.size();
        N_veto_ele=selector->ElectronsLoose.size();
   // FillVetoLeptonHist();
   // if(selector->ElectronsLoose.size()>=1){
   // TList *list = (TList *)fFileDir[1]->GetList();
     // FillCFHists(list, "", false, 6.0, 1.0, 1.0);}
 
    //cout<<"N_veto_ele="<<N_veto_ele<<"  N_veto_mu="<<N_veto_mu<<endl;

  //cout<<"Loose electron size="<<selector->ElectronsLoose.size()<<"    Loose Muon size="<<selector->MuonsLoose.size()<<endl;  
...
//if(_nTau>0)
  ...
  doubleTau = (evtPick->passFilter and selector->isPVGood and evtPick->passTrigger_tau and evtPick->passPresel_tau and selector->Taus.size() >= 2); //For Tau Analysis
  ...
  bool pass_PairCharge_and_dR =false;
  if(_nTau>=2){
    ...
    for (int i_tau = 0; i_tau <_nTau; i_tau++){
 	...
 	if(dR(event->tauEta_[tauIndex_1], event->tauPhi_[tauIndex_1], event->tauEta_[tauIndex_2], event->tauPhi_[tauIndex_2]) > 0.5 && (event->tauCharge_[tauIndex_1])*(event->tauCharge_[tauIndex_2])==-1){
	pass_PairCharge_and_dR=true;
    ...
    sum_pt = sortedDiTaus.sumPt_;
    ditau_charge = sortedDiTaus.diTauCharge_;
    tauId_1 = sortedDiTaus.index1_ ;
    tauId_2 = sortedDiTaus.index2_ ;
    //cout << "ditau mass=" << ditau_m << endl;                                                                                      
    //cout<< "test=66"<<endl;
    cout<<"This is from SkimAna"<<endl;
    cout<<"Tau_1 Id="<<tauId_1<<"  Tau_2 Id="<<tauId_2<<endl;
  ...
  doubleTau = (doubleTau and  _nTau >= 2 and pairCharge == -1);
  ...
  Ele_Muon_veto =false;
      if((selector->ElectronsLoose.size() + selector->Electrons.size())>=1 ||(selector->MuonsLoose.size() + selector->Muons.size())>=1)
	Ele_Muon_veto =true;
	
   if(Ele_Muon_veto) return true;
...
*/
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////////////////
 Ele_Muon_veto =false;
      if((selector->ElectronsLoose.size())>=1 ||(selector->MuonsLoose.size())>=1)
        Ele_Muon_veto =true;
        
   if(Ele_Muon_veto) return true;
  if(systType == kBase) FillLeptonVetoControlHists();
  if(IsDebug) Info("Process","Completed Lepton Veto processing");
  //FillVetoLeptonHist();
 
  //######################################################
  selector->filter_jets(event, tauId_1, tauId_2);
 ...
 // FillNjetCutFlow();
  //FillJetHist();
 int count_dR=0;
        if(selector->Jets.size() >= 6 ){
                for (int i_jet = 0; i_jet <selector->Jets.size(); i_jet++){
                int jetIndex_1 = selector->Jets.at(i_jet);
                for(int j_jet = i_jet+1 ; j_jet <selector->Jets.size(); j_jet++){
                        if(j_jet<=i_jet) continue;
                int jetIndex_2 = selector->Jets.at(j_jet);
                if(dR(event->jetEta_[jetIndex_1], event->jetPhi_[jetIndex_1], event->jetEta_[jetIndex_2], event->jetPhi_[jetIndex_2]) > 0.4)
                     count_dR=count_dR+1;
                }
                }
        }

 bool JetJet_dR =false;
 int n_jet = 0;
 n_jet = selector->Jets.size();
 JetJet_dR =(n_jet==6 && count_dR==15)||(n_jet==7 && count_dR>=20)||(n_jet>=8 && count_dR>=25);
 if(!JetJet_dR) return true;
 //FillJetHist();
 //FillNjetCutFlow();
//////=====================================================
...
bool SkimAna::FillCFHists(TList *list, string hist_extn, bool isMu, double value, double wt, double wt_nobtag){
  ...
  // }else{
    ...
    ((TH1D *) list->FindObject(Form("_cutflow_tau%s",hist_extn.c_str())))->Fill(value,wt);
    /*
    if(isMu){
...
bool SkimAna::FillNjetCFHists(int isyst, double combined_muwt, double combined_muwt1, double combined_elewt, double combined_tauwt)
{
  if(selector->Jets.size() >=6){
    TList *list = (TList *)fFileDir[isyst*fNDDReg + 0]->GetList();                                                                       
    FillCFHists(list, "", false, 5.0, combined_tauwt, 1.0); 
    //cout<<"Trigger, PVGood, DiTau, Njets>=6:The event="<<event->event_<<endl;
    if(selector->bJets.size() >=2){
      TList *list = (TList *)fFileDir[isyst*fNDDReg + 0]->GetList();
      FillCFHists(list, "", false, 6.0, combined_tauwt, 1.0);
      //cout<<"Trigger, PVGood, DiTau, Njets>=6,BJet>=2:The event="<<event->event_<<endl;
    }
  }  
/*
  if(singleMu and muonIsoCut){
...
bool SkimAna::FillNjetCutFlow(){
  ...
  for(int isyst=0;isyst<fNSyst;isyst++){
	...
	cout<<"_sampleWeight="<<_sampleWeight<<"  _prefireWeight="<<_prefireWeight<<"  _PUWeight="<<_PUWeight<<"  combined_tauwt="<<combined_tauwt<<endl;
...
bool SkimAna::FillLeptonCFHists(int isyst, double combined_muwt, double combined_muwt1, double combined_elewt, double combined_tauwt){
  ...
  if(doubleTau){         
   ...
   // cout<<"Trigger, PVGood, DiTau:The event="<<event->event_<<endl;
  	if(!(selector->ElectronsLoose.size()>=1 || selector->MuonsLoose.size()>=1)){
...
bool SkimAna::FillEventCFHists(int isyst, double combined_muwt, double combined_muwt1, double combined_elewt, double combined_tauwt){
  ...
  */
  ...
  if(evtPick->passPresel_tau)
    ...
    if(evtPick->passPresel_tau and evtPick->passTrigger_tau and selector->isPVGood and evtPick->passFilter)
...
////////added new one to fill all histogram............
bool SkimAna::FillVetoLeptonHist(){

  for(int isyst=0;isyst<fNSyst;isyst++){
    TList *list = (TList *)fFileDir[isyst*fNDDReg + 0]->GetList();
    ((TH1D *) list->FindObject("nEleLoose"))->Fill(N_veto_ele);
    ((TH1D *) list->FindObject("nMuLoose"))->Fill(N_veto_mu);
...
bool SkimAna::FillTauHist(){
  
  for(int isyst=0;isyst<fNSyst;isyst++){
    TList *list = (TList *)fFileDir[isyst*fNDDReg + 0]->GetList();
    ((TH1D *) list->FindObject("mass_ditau"))->Fill(ditau_m);
    ((TH1D *) list->FindObject("charge_ditau"))->Fill(ditau_charge);
    ((TH1D *) list->FindObject("Sum_Pt_ditau"))->Fill(sum_pt);

    ((TH1D *) list->FindObject("pt_tau_1"))->Fill(event->tauPt_[tauId_1]);
    ((TH1D *) list->FindObject("eta_tau_1"))->Fill(event->tauEta_[tauId_1]);
    ((TH1D *) list->FindObject("phi_tau_1"))->Fill(event->tauPhi_[tauId_1]);
    ((TH1D *) list->FindObject("charge_tau_1"))->Fill(event->tauCharge_[tauId_1]);
    
    ((TH1D *) list->FindObject("pt_tau_2"))->Fill(event->tauPt_[tauId_2]);
    ((TH1D *) list->FindObject("eta_tau_2"))->Fill(event->tauEta_[tauId_2]);
    ((TH1D *) list->FindObject("phi_tau_2"))->Fill(event->tauPhi_[tauId_2]);
    ((TH1D *) list->FindObject("charge_tau_2"))->Fill(event->tauCharge_[tauId_2]);

    ((TH1D *) list->FindObject("nTau"))->Fill(selector->Taus.size());
...
bool SkimAna::FillJetHist(){

  for(int isyst=0;isyst<fNSyst;isyst++){
    TList *list = (TList *)fFileDir[isyst*fNDDReg + 0]->GetList();
    ((TH1D *) list->FindObject("nJet_test1"))->Fill(selector->Jets_test1.size());
    ((TH1D *) list->FindObject("nJet_test2"))->Fill(selector->Jets_test2.size());
    ((TH1D *) list->FindObject("nJet_test3"))->Fill(selector->Jets_test3.size());
    ((TH1D *) list->FindObject("nJet"))->Fill(selector->Jets.size());
  }

  return true;
}

bool SkimAna::FillBjetHist(){

  for(int isyst=0;isyst<fNSyst;isyst++){
    TList *list = (TList *)fFileDir[isyst*fNDDReg + 0]->GetList();
    ((TH1D *) list->FindObject("nBJet"))->Fill(selector->bJets.size());
...
bool SkimAna::FillLeptonControlHists(){
    ...
    ((TH1D *) list->FindObject("mass_ditau"))->Fill(ditau_m,combined_tauwt);
    ((TH1D *) list->FindObject("charge_ditau"))->Fill(ditau_charge,combined_tauwt);
    ((TH1D *) list->FindObject("Sum_Pt_ditau"))->Fill(sum_pt,combined_tauwt);

    ((TH1D *) list->FindObject("pt_tau_1"))->Fill(event->tauPt_[tauId_1],combined_tauwt);
    ((TH1D *) list->FindObject("eta_tau_1"))->Fill(event->tauEta_[tauId_1],combined_tauwt);
    ((TH1D *) list->FindObject("phi_tau_1"))->Fill(event->tauPhi_[tauId_1],combined_tauwt);
    ((TH1D *) list->FindObject("charge_tau_1"))->Fill(event->tauCharge_[tauId_1],combined_tauwt);

    ((TH1D *) list->FindObject("pt_tau_2"))->Fill(event->tauPt_[tauId_2],combined_tauwt);
    ((TH1D *) list->FindObject("eta_tau_2"))->Fill(event->tauEta_[tauId_2],combined_tauwt);
    ((TH1D *) list->FindObject("phi_tau_2"))->Fill(event->tauPhi_[tauId_2],combined_tauwt);
    ((TH1D *) list->FindObject("charge_tau_2"))->Fill(event->tauCharge_[tauId_2],combined_tauwt);

    ((TH1D *) list->FindObject("nTau"))->Fill(selector->Taus.size(),combined_tauwt);
