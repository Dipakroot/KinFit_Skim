...
Transfer_Input_Files = LQAnalysis.tar.gz, run_LQAnalysis.sh\n\
...
for year in [2018]:
    ...
    jdlFile.write('Executable =  run_LQAnalysis.sh \n')
    jdlFile.write(common_command)
    condorOutDir1="/eos/user/d/dmaity/condor_job/LQAnalysis/Background/hist"
    condorOutDir2="/eos/user/d/dmaity/condor_job/LQAnalysis/Background/bjet"
    condorOutDir3="/eos/user/d/dmaity/condor_job/LQAnalysis/Background/tree"
 
    for sl in sampleList:
        sample_ =sl
        os.system("eos root://eosuser.cern.ch mkdir -p %s/%s/%s"%(condorOutDir1,sample_,year))
        os.system("eos root://eosuser.cern.ch mkdir -p %s/%s/%s"%(condorOutDir2,sample_,year))
        os.system("eos root://eosuser.cern.ch mkdir -p %s/%s/%s"%(condorOutDir3,sample_,year))
